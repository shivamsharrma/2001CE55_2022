...
def octant_analysis(mod=5000):
   #code to read input csv file and store that in dataframe
    path= os.getcwd()
    filenames=glob.glob(os.path.join(path,"input\\*.xlsx"))
    
    file_no=0
    input_file_name=[1.0,2.0,2.5,3.0,3.4,3.7,4.0,4.2,4.4,4.6,4.7,4.8,4.9,5.0,5.1,5.2,5.3,"surface"]
    for f in filenames:
        df=pd.read_excel(f)
        df=df.fillna(0) 
    ...
    #takingh mean of each 'U','V','W' column
        ...
        while c<n:
            ...
            try:
                ...
                print('There is error in counting longest subsequence length\n')
        ...
        for k in range(T):
            ...
            rowloc+=rownum+dis
        wb.save("output\\{}.xlsx".format(input_file_name[file_no]))
        file_no+=1
