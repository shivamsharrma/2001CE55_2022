...
sch=0
# def handle_compute(constant_fk2d,multiplying_factor_3d,Shear_velocity,tch,corr,SNR,Lambda,k,sch):
    
st.title('DTA TO CSV FILE CONVERTER')
# with st.form(key='my_form2'):


constant_fk2d=st.number_input("constant_fk2d",key='constant')
multiplying_factor_3d=st.number_input("multiplying_factor_3d",key='multiplefactor')
Shear_velocity=st.number_input("Shear_velocity",key='velocity')
Shear_velocity=Shear_velocity**3

if((st.session_state['constant']!=0)&(st.session_state['multiplefactor']!=0)&(st.session_state['velocity']!=0)):
    file1=st.header("Enter The Following Value")
    list1=['1. C','2. S','3. A','4. C & S','5. C & A','6. S & A','7. C & S & A','8. all combine']
    
    for i  in list1:
        st.write(i)
    tch = st.selectbox("Choose the filtering method from above", options=('Select here',1,2,3,4,5,6,7,8), key='tch') # Creates a selectbox where user has to select the filtering method with key 'tch'

    if st.session_state['tch'] == 1: # If-else statements that ask for further inputs based on session state of key 'tch' i.e what is the value of tch in the current session, the default value of key 'tch' is 'Select here' which promts the user to select a filtering method
        corr = int(st.number_input("Enter thresold value C:")) 
    if st.session_state['tch'] == 2:
        SNR = int(st.number_input('Enter thresold value S:'))
    if st.session_state['tch'] == 3:
        Lambda = float(st.number_input('Enter Lambda value for A:'))
        k = float(st.number_input('Enter k value for A:'))
    if st.session_state['tch'] == 4:
        corr = int(st.number_input('Enter thresold value C:'))
        SNR = int(st.number_input('Enter thresold value S:'))
    if st.session_state['tch'] == 5:
        corr = int(st.number_input('Enter thresold value C:'))
        Lambda = float(st.number_input('Enter Lambda value for A:'))
        k = float(st.number_input('Enter k value for A:'))
    if st.session_state['tch'] == 6:
        SNR = int(st.number_input('Enter thresold value S:'))
        Lambda = float(st.number_input('Enter Lambda value for A:'))
        k = float(st.number_input('Enter k value for A:'))
    if st.session_state['tch'] == 7 or st.session_state['tch'] == 8:
        ...
        Lambda = float(st.number_input('Enter Lambda value for A:'))
        k = float(st.number_input('Enter k value for A:'))
    elif st.session_state['tch'] ==  'Select here': # If no filtering method is selected
        st.warning("Select a filtering method") # A warning is given to user to select a method


    
    if type(st.session_state['tch']) == int:

        list2=['1. previous point','2. 2*last-2nd_last','3. overall_mean',
        '4. 12_point_strategy','5. mean of previous 2 point',
        '6. all seqential','7. all parallel']  
        st.subheader("Select The Replacement Method")
        
        for i in list2:
            st.text(i)
        sch=st.selectbox('Chose Replacement Method From Above:',options=('Select here',1,2,3,4,5,6,7,8), key='sch')
        with st.form("files"): # Form for computing result
            global comp
            comp=st.form_submit_button("COMPUTE")
            if(comp):
    ...
    if sch!=0:
        if st.session_state['FormSubmitter:files-COMPUTE'] ==True:  
            with ZipFile('convertedfile.zip', 'w') as zip_object:
            
                for file in files: # Looping over files
                    input_filename = file.strip()
                    zip_object.write(f'{input_filename[:-4]}.csv') 
                zip_object.write('Results_v2.csv') 
            st.markdown("---")
            
            with open('convertedfile.zip', 'rb') as converted_file: 
                btn = st.download_button(
                    label='Download converted files', 
                    data=converted_file, 
                    file_name='convertedfile.zip' 
                )